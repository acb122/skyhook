...
export function cardReducer(state: BoardState, action: SortCard) {
    ...
    switch (action.event) {
        ...
        case SortableEvents.Drop: {
            return copyOrInsertCard(
                state,
                cloneCard(state.cardInFlight.data, state.nextId)
            );
        }
        case SortableEvents.EndDrag: {
            return resetDrag(state);
...
export const _cardInFlight = createSelector(
    _boardState,
    (state) => state.cardInFlight
);
...
export const _temporaryClone = createSelector(
    ...
    _cardInFlight,
    (copying, card) =>
        copying && card ? cloneCard(card.data, CARD_ID_WHEN_COPYING) : null
);

const _board = createSelector(_boardState, (state) => state.board);
export const _listById = (listId: any) =>
    createSelector(_board, (board) => {
