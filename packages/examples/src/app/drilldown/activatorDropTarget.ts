import { SkyhookDndService, DropTargetSpec } from '@rednax-skyhook/core';
...
export type ActivatorSpec = DropTargetSpec & {
...
export function activatorDropTarget(
    ...
    const dt = dnd.dropTarget(types, {
        ...
        hover: (monitor) => {
            hoverSubject$.next(
                new ActivatedWith(monitor.getItemType(), monitor.getItem())
            );
