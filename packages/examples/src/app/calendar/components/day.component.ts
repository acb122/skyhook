...
import { SkyhookDndService, DragSourceOptions } from '@rednax-skyhook/core';
...
@Component({
    ...
    template: `
        <div
            class="day"
            [class.day--othermonth]="otherMonth$ | async"
            [class.day--weekend]="isWeekend"
        >
            <div class="day-pad day-pad--bg"></div>

            <h3 class="day-label">
                <span
                    class="day-label-lozenge"
                    [class.day-label-lozenge--today]="isToday"
                    >{{ day.getDate() }}</span
                >
            </h3>
...
export class CalendarDayComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    myEvents = createSelector(allEventSelector, (es) => {
        return es.filter((e) => {
            const startOfAllday =
                e.isAllDay && e.start.valueOf() === this.day.valueOf();
                ...
                switch (type) {
                    case ItemTypes.EXISTING: {
                        return this.store.dispatch(
                            new HoverExistingEvent(
                                id,
                                daysBetween(start, this.day)
                            )
                        );
                    }
                    case ItemTypes.RESIZE_START: {
                        return this.store.dispatch(
                            new HoverResizeStart(
                                id,
                                daysBetween(start, this.day)
                            )
                        );
                    ...
                    case ItemTypes.RESIZE_END: {
                        return this.store.dispatch(
                            new HoverResizeEnd(id, daysBetween(end, this.day))
                        );
                    }
