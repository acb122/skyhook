...
export class SkyhookSortableRenderer<Data> implements OnInit, OnDestroy {
    ...
    private hover(monitor: DropTargetMonitor<DraggedItem<Data>>): void {
        ...
        if (suggestedIndex < 0) {
            ...
            // suggestedIndex = 0;
            throw new Error(
                '@rednax-skyhook/sortable: Cannot move a card to an index < 0.'
            );
        }
    ...
    private rect() {
        if (!this.el) {
            throw new Error(
                '@rednax-skyhook/sortable: [ssRender] expected to be attached to a real DOM element'
            );
        }
